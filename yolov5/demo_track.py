...
def write_results(filename, results):
    ...
    with open(filename, 'w') as f:
        for frame_id, tlwhs, track_ids, scores, cls_id in results:
...
class Predictor(object):
    ...
    def inference(self, img, timer):
        ...
        with torch.no_grad():
            ...
            global infer_time
            infer_time += time.time() - start3
            # print(outputs)
            """
            torch.Size([38760, 7])：这个输出表示有38760个检测框，每个检测框有7个值。通常，这些值包括检测框的坐标（x、y、宽度、高度），以及相关的类别和置信度信息。

            torch.Size([1, 3, 76, 136, 7])：这个输出表示有1个批次（batch）的数据，其中包含3个不同的尺度的检测结果。对于每个尺度，输出的维度为(76, 136, 7)，表示在这个尺度上有76行、136列的特征图，每个位置上有7个值，类似于上述的含义。

            torch.Size([1, 3, 38, 68, 7])：这个输出与上一个输出类似，表示另一种尺度上的检测结果。在这个尺度上，特征图的大小为38行、68列。
            """
            # for i in outputs:
            #     for j in i:
            #         print(j.shape)
            ...
            # print('before nms:', torch.tensor(outputs[0]).shape)
            ...
            global nms_time
            nms_time += time.time() - start4
            # print(outputs) # [torch.Size([138, 7])]
            ...
            # print('after nms:', torch.tensor(outputs[0]).shape)
...
def save_outputs(outputs, folder, save_name):
    ...
    root_dir = r'F:\work_space\pe_car_detect\yolov5v6_bytetrack'  # 获取当前工作目录

    # 创建输出的偏移量 类别， 置信度 的txt文件
    sn = os.path.join(root_dir,'runs',folder, sn)
...
def image_demo(predictor, vis_folder, path, current_time, save_result, save_name, test_size):
    ...
    for image_name in files:
        ...
        if outputs[0] is not None:
            ...
            # print(type(online_targets[0]))  # <class 'tracker.byte_tracker.STrack'>
            ...
            # print('online_cls', online_cls)
            # save results
            results.append((frame_id + 1, online_tlwhs, online_ids, online_scores, online_cls))
            timer.toc()
            online_im = plot_tracking(img_info['raw_img'], online_tlwhs, online_ids, frame_id=frame_id + 1,
                                          total_time=timer.average_time, cls_id=online_cls, scores=online_scores)
        else:
            timer.toc()
            online_im = img_info['raw_img']

        global post_time
        post_time += time.time() - start5
...
def main(args):
    ...
    print('past_time, track_time, nms_time, post_time, infer_time',past_time, track_time, nms_time, post_time+nms_time, infer_time)
    print('avg', past_time/181., track_time/181., nms_time/181., (post_time + nms_time)/181., infer_time/181.)
